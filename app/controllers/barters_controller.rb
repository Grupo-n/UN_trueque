    ...
    respond_to do |format|
      if @barter.update(barter_params)
        if @barter.accept_user_one == 'false' and @barter.accept_user_two == 'false'
          ...
          SendMailersJob.set(wait: 10.seconds).perform_later(@barter, @barter.get_user_one, @barter.get_user_two)
  ...
  private
    ...
    def barter_params
      params.require(:barter).permit(:description, :product_one_id, :product_two_id,
        :state, :confirmation, :id_one_user, :id_two_user, :accept_user_one, :accept_user_two,
        :meeting_date, :meeting_time, :latitude, :longitude
)
